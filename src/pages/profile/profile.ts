  ...
  updateUser(uid) {
    ...
    this.usersService.editUser(auxUser).then((response) => {
      ...
      if(updatePassword) {
        ...
        user.updatePassword(auxUser.password).then((result) => {
          console.log(result);
          ...
          if (error && error.code === 'auth/requires-recent-login') {
            alert('Actualizar la contraseña requiere de un login reciente. Por favor cierra sesión, vuelve a ingresar e intenta de nuevo.');
          }
        });
      }
      this.loadingService.dismiss();
