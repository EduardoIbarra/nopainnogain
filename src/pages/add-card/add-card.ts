...
export class AddCardPage {
  ...
  cleanFormOperation() {
    this.cardData.deviceSessionId = OpenPay.deviceData.setup("payment-form", "deviceIdHiddenFieldName");
    OpenPay.token.extractFormAndCreate('payment-form', (response) => {
      console.log(response);
      debugger;
      this.cardData.token_id = response.data.id;
      this.purchase();
    }, (error) => {
      console.log(error);
    });
  ...
  purchase() {
    ...
    this.paymentService.GymPayment(this.cardData, this.sharedService.UserData, '45').subscribe((response) => {
      console.log(response);
